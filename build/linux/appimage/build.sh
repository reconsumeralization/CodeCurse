...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's|@@NAME@@|CodeCurse-Insiders|g' recipe.yml
    sed -i 's|@@APPNAME@@|codecurse-insiders|g' recipe.yml
    sed -i 's|@@ICON@@|codecurse-insiders|g' recipe.yml
  else
    sed -i 's|@@NAME@@|CodeCurse|g' recipe.yml
    sed -i 's|@@APPNAME@@|codecurse|g' recipe.yml
    sed -i 's|@@ICON@@|codecurse|g' recipe.yml
  fi
